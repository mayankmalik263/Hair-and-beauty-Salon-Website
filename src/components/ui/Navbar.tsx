...
export function Navbar() {
  ...
  return (
    <>
      <header className="sticky top-0 z-50 w-full border-b border-white/10 bg-black/80 backdrop-blur-md">
        <div className="container mx-auto flex h-20 items-center justify-between px-4 md:px-6">
            ...
            {navLinks.slice(1).map((link) => {
              ...
              return (
                <Link
                  ...
                  className={`p-2 transition-colors outline-none focus-visible:ring-2 focus-visible:ring-gold-500 rounded-md ${
                    isActive ? "text-gold-400" : "text-zinc-300 hover:text-gold-400"
                  }`}
                >
                  {link.name}
                </Link>
          ...
          <div className="hidden lg:flex items-center gap-4">
            <a
              href="tel:8222830906"
              className="flex items-center gap-2 p-2 text-sm font-medium transition-colors text-zinc-300 hover:text-gold-400 outline-none focus-visible:ring-2 focus-visible:ring-gold-500 rounded-md"
              aria-label="Call us"
            >
      ...
      {/* Mobile Menu Overlay */}
      {isMobileMenuOpen && (
        <div className="fixed inset-0 z-[60] flex flex-col bg-black/95 backdrop-blur-xl lg:hidden">
          ...
          <div className="flex flex-col flex-1 px-4 py-8 overflow-y-auto">
                ...
                return (
                  <Link
                    ...
                    className={`flex items-center min-h-[56px] px-4 text-lg font-medium rounded-lg transition-colors outline-none focus-visible:ring-2 focus-visible:ring-gold-500 ${
                      isActive ? "bg-gold-500/10 text-gold-400" : "text-zinc-300 hover:bg-white/5 hover:text-white"
                    }`}
                  >
                    {link.name}
                  </Link>
            ...
            <div className="mt-auto flex flex-col gap-4">
              <a
                href="tel:8222830906"
                className="flex items-center justify-center gap-2 min-h-[56px] px-4 text-lg font-medium border border-white/10 rounded-lg text-zinc-300 hover:bg-white/5 transition-colors outline-none focus-visible:ring-2 focus-visible:ring-gold-500"
              >
              ...
              <Link 
                href="/book" 
                className={buttonVariants({ variant: "primary", size: "lg", className: "w-full text-lg" })}
              >
                Book Appointment
              </Link>
            </div>
